			...
			<td>
				<?php 
					...
					case 'number': ?>
						...
						<?=$field['label']=='Price'? '$' : '' ?><input type="number" name="<?=$field['id']?>" id="<?=$field['id']?>" value="<?=((isset($meta)) ? $meta : '')?>" size="50" <?=(($field['required'])? 'required' :'')?>/><?=$field['label']=='Pass score (%)'? '%' : '' ?><br />
						<span class="description"><?=$field['desc']?></span>
...
<?php wp_nonce_field('save_test_post','save_test_post'); ?>