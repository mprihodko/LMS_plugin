...
#####################################################################################################################
	...
	public function save_test($post_id) {		
		if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ) 
			return $post_id;
		if(!isset($_POST['save_test_post']) || !wp_verify_nonce($_POST['save_test_post'],'save_test_post'))
			return $post_id;
		...
		if($product_id){
			update_post_meta($post_id, "lms_product_id", $product_id);
			update_post_meta($product_id, "_lms_price",  $_POST['test_price']);
			update_post_meta($product_id, "_lms_test_id", $post_id);
			update_post_meta($product_id, "_lms_product_type", "tests");
		}
		/*save quests*/
	...
	public function deleteTest($post_id){
		if(get_post_type( $post_id )=="lms_test"){	
			$childrens = get_post_meta($post_id, "lms_product_id", true);			
			wp_delete_post($childrens, true);	
